...
- **Saved CSVs store intent plus SQL and belong to one database context.** After a successful export, users may save a reusable CSV. The normal UX names and manages the saved CSV plan, while the app persists the approved intent, validated SQL, and scanned context fingerprint internally. Reruns skip chat/model generation but require a matching database context and never skip SQL validation or read-only execution limits.
