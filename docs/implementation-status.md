...
Follow-up narrowing on 2026-05-03 removed the unused public `/sessions/{session_id}/clarify` and `/sessions/{session_id}/propose-sql` endpoints from the HTTP/OpenAPI surface. Clarification still happens through `/sessions/{session_id}/propose-intent` with `intent: null`, and SQL generation/repair stays on the approval-gated `/sessions/{session_id}/prepare-sql` path. SQL prompts now explicitly tell the model to satisfy any approved `source_hint` by selecting that `table.column` and aliasing it to the approved CSV column name.
...
Last documented full backend suite: `134 passed` on 2026-05-03 after removing unused public clarify/propose-SQL endpoints while keeping the approval-gated prepare path.
...
Last documented frontend verification: focused `pnpm test -- --run src/App.test.tsx` passed with `10 passed`; `pnpm build` passed on 2026-05-03 after SQL-preparation retry coverage and API type regeneration.
