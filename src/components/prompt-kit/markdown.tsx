...
const INITIAL_COMPONENTS: Partial<Components> = {
  code: function CodeComponent({ className, children, ...props }) {
    ...
    if (isInline) {
      return (
        <span className={cn("rounded-sm bg-muted px-1 font-mono text-sm", className)} {...props}>
          {children}
        </span>
      );
    }
...
const MemoizedMarkdownBlock = memo(
  function MarkdownBlock({ content, components = INITIAL_COMPONENTS }: { content: string; components?: Partial<Components> }) {
    return (
