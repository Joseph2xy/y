  ...
  return (
    <TooltipProvider>
      ...
      >
        <div
          onClick={handleClick}
          className={cn("cursor-text rounded-3xl border border-input bg-background p-2 shadow-xs", disabled && "cursor-not-allowed opacity-60", className)}
          {...props}
        >
          {children}
        </div>
...
function PromptInputAction({ tooltip, children, className, side = "top", ...props }: PromptInputActionProps) {
  ...
  return (
    <Tooltip {...props}>
      <TooltipTrigger disabled={disabled} onClick={(event) => event.stopPropagation()} render={children} />
